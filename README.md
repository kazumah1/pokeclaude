...
| Kiro IDE | an **editor tab** — its panel will not display a local image, but its editor previews one properly |
| Any **catch** | an **editor tab** — a hook fires after the agent stops speaking, so nothing can echo a link |
